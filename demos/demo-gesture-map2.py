...
def process(frame):
    global current_gesture, gesture_start_time, current_pressed_key  # Include the new global variable
    ...
    if recognition_result.gestures:
        ...
        if current_gesture != top_gesture[0].category_name:
            ...
            if current_pressed_key:
                controller.release(current_pressed_key)
                current_pressed_key = None
            
            current_gesture = top_gesture[0].category_name
            gesture_start_time = time.time()
        elif time.time() - gesture_start_time >= TIME_RECOGNITION and current_gesture != "None":
            # If the gesture is maintained and we haven't already pressed the key
            if current_pressed_key is None:
                print(f"Gesture '{current_gesture}' has been maintained for {TIME_RECOGNITION} seconds!")
                gesture_key = GESTURES_MAPPED[current_gesture]
                print(f'Pressing: {gesture_key}')
                controller.press(gesture_key)
                current_pressed_key = gesture_key  # Remember the key we've pressed

    else:
        # If no gesture is detected and we have a key pressed, release it
        if current_pressed_key:
            controller.release(current_pressed_key)
            current_pressed_key = None
        current_gesture = None
